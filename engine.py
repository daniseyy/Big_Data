...
class RecommendationEngine:
    ...
    def __init__(self, spark_session, dataset_path):
        ...
        logger.info("Loading Artists Alias data...")
        ratings_file_path = os.path.join(dataset_path, 'artist_alias_small.txt')
        self.ratingsdf = spark_session.read.csv(artist_alias_file_path, header=True, inferSchema=True).na.drop()
        
        # Load movies data for later use
        logger.info("Loading Artists data...")
        movies_file_path = os.path.join(dataset_path, 'artist_data_small.txt')
        self.moviesdf = spark_session.read.csv(artist_file_path, header=True, inferSchema=True).na.drop()
       
       logger.info("Loading User Artists data...")
        movies_file_path = os.path.join(dataset_path, 'user_artist_data_small.txt')
        self.moviesdf = spark_session.read.csv(user_file_path, header=True, inferSchema=True).na.drop()
